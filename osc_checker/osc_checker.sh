...
#  - wget
# invocation:
#  ./osc_checker.sh {filename.osc or sequence number}
#
...
if [ ! -f "${input}" -a "${input}" -gt 1 ]
	...
	wget -O "${tmpdir}/${input}" ${geofabrik_url}/${input}.osc.gz
	input="${tmpdir}/${input}"
	if [ ! -s "${input}" ]
		then
