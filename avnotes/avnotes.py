...
audiofiles = [x for x in files if re.search(r"\.3gp$", x)]
...
for string in [re.sub("(.*)\.3gp", r"\1", x) for x in audiofiles]:
    basename = string
...
for wpt in waypoints:
    ...
    print("<time>" + str(datetime.fromtimestamp(time)).replace(" ", "T") + "Z" + "</time>")
